import { PurchaseModel } from "../subscription/subscription.model";
import { getSubscriptionStats } from "../subscription/subscription.service";
...
const getMonthRanges = (months: number) => {
...
const countActivePlanAt = async (plan: "VIP" | "Forex" | "Crypto", end: Date) => {
  return PurchaseModel.countDocuments({
...
const buildSparkline = (values: number[]) => values.map((v) => ({ v }));

const getGrowthData = async (months: number) => {
  const ranges = getMonthRanges(months);
  const growthData = [];

  for (const range of ranges) {
    const [vip, forex, crypto] = await Promise.all([
...
const getRevenueData = async (months: number) => {
  const ranges = getMonthRanges(months);
  const revenueData = [];
...
const getSignalPerformance = async () => {
  const closedSignals = await SignalModel.find({ status: "Closed" }).select("closeResult");
  ...
  const toPercent = (value: number) =>
    counted > 0 ? Math.round((value / counted) * 100) : 0;
...
const getActivityFeed = async () => {
  const [purchases, signals, posts, notifications, trialUsers, coupons] =
    await Promise.all([
      PurchaseModel.find({ paymentStatus: "completed" })
        .sort({ createdAt: -1 })
        .limit(5)
        .populate("userId", "firstName lastName name email"),
      SignalModel.find({ status: { $in: ["Active", "Closed"] } })
        .sort({ publishedAt: -1, createdAt: -1 })
        .limit(5),
      PostModel.find({ status: "Published" }).sort({ publishedAt: -1 }).limit(5),
      NotificationModel.find({ status: "Sent" }).sort({ sentAt: -1 }).limit(5),
      UserModel.find({
        role: "user",
        isDeleted: false,
      ...
      CouponModel.find({ used: { $gt: 0 } }).sort({ updatedAt: -1 }).limit(5),
    ]);
...
export const getDashboardStats = async (range: RangeKey = "6M") => {
  const months = range === "7D" ? 1 : range === "30D" ? 2 : 6;
  const subscriptionStats = await getSubscriptionStats();
  const growthData = await getGrowthData(months);
  const revenueData = await getRevenueData(months);
  const signalPerformance = await getSignalPerformance();
  const marketTickers = await getMarketTickers();
  const activityFeed = await getActivityFeed();

  const currentMonthRevenue = revenueData[revenueData.length - 1]?.r || 0;
  const previousMonthRevenue = revenueData[revenueData.length - 2]?.r || 0;
  ...
  const totalSpark = buildSparkline(
    growthData.map((item) => item.v + item.f + item.c),
  );
  ...
  return {
    ...
    kpis: {
      total: {
        ...
        change: calcChange(
          currentGrowth.v + currentGrowth.f + currentGrowth.c,
          previousGrowth.v + previousGrowth.f + previousGrowth.c,
        ),
